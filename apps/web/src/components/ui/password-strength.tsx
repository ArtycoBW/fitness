...
import { passwordRules, isGuessablePassword } from "@fitness/validation";
...
export const defaultPasswordRules: readonly PasswordRule[] = passwordRules;
const defaultLabels = [
  "Введите пароль",
  "Слабый",
  "Средний",
...
export function usePasswordStrength(
  ...
  const state = useMemo(() => {
    ...
    const guessable = value.length > 0 && isGuessablePassword(value);
    ...
    const announcement =
      value.length === 0
        ? ""
        : [
            `Надёжность пароля: ${label.toLowerCase()}.`,
            guessable ? "Такой пароль легко угадать." : "",
            unmet.length === 0
              ? "Все требования выполнены."
              : `Ещё требуется: ${unmet.map((r) => r.label.toLowerCase()).join(", ")}.`,
          ]
            .filter(Boolean)
            .join(" ");
...
const TONES = {
  ...
  caution: { bar: "bg-amber-500", text: "text-amber-800 dark:text-amber-400" },
  safe: {
    bar: "bg-emerald-500",
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="mt-2 flex h-5 items-center justify-between gap-3">
        ...
        <motion.span
          aria-hidden
          className="whitespace-nowrap text-[11.5px] leading-5 text-amber-800 dark:text-amber-400"
          initial={false}
          ...
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              ...
              <span className="sr-only">
                {rule.met ? "выполнено" : "не выполнено"}
              </span>
            </li>
          ))}
        </ul>
